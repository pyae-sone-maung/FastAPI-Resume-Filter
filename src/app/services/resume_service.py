import re
import fitz
from src.app.utils import file_utils
import pytesseract
...
class ResumeService:
    async def get_porcessed_resume_data(self, upload_file):
        extract_text = ""
        ext = file_utils.get_file_extension(upload_file)
        ...
        try:
            ...
            logger.error(f"Error in get_porcessed_resume_data: {ex}")
            return None
    ...
    def _extract_text_from_docx(self, docx_file) -> str | None:
        try:
            ...
            logger.error(f"Error during text extract for docx file: {ex}")
            return None
