from src.app.ai_engine.core.ai_factory import AIModelFactory
...
class AIEngine:
    def __init__(self):
        ...
        self._current_model: Union[AbstractTextModel, AbstractImageModel] = None
        ...
        if self._current_model_name != model_name:
            ...
            if model_name not in self.initialize_model:
                try:
                    ...
                    logger.error(f"Error initializing model {model_name} : {ex}")
                    raise ValueError(f"Error initializing model {model_name} : {ex}")
            self._current_model = self.initialize_model[model_name]
            self._current_model_name = model_name
        else:
            logger.info(f"Already active model : {model_name}")
            
    def get_active_model(self) -> Union[AbstractTextModel, AbstractImageModel]:
    ...
    def generate_text(self, prompt, model_name, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error generating text for model {model_name} : {ex}")
